...
void insert(Node *&curr,int val,int out[]){
   if (!checkPerfectBalance(curr) || !checkPerfectBalance(curr)){ 
       
       int l=SortedArrayFromBST(curr,out,0); //deleteNode(curr);
  ...
  for(int j=0;j<queries;j++){
    ...
    //if(c==1){
      ...
      if(root){
        ...
        cur_t=(float)(end-start)/(CLOCKS_PER_SEC);
        if(prev_time<cur_t)
          cout<<j<<" "<<cur_t<<endl;
        prev_time=(float)(end-start)/(CLOCKS_PER_SEC);
